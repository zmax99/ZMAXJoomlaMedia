...
FileProgress.prototype.setChunkProgess = function(chunk_size) {
    ...
    var viewProgess = jQuery('<button class="btn btn-default">查看分块上传进度</button>');

    var progressBarChunkTr = jQuery('<tr class="chunk-status-tr"><td colspan=3></td></tr>');
    var progressBarChunk = jQuery('<div/>');
    for (var i = 1; i <= chunk_amount; i++) {
        var col = jQuery('<div class="span2"/>');
        var progressBarWrapper = jQuery('<div class="progress progress-striped"></div');
        ...
        var progressBarStatus = jQuery('<span/>');
...
FileProgress.prototype.setProgress = function(percentage, speed, chunk_size) {
    ...
    if (chunk_size) {
        ...
        for (var index = 0; index < current_uploading_chunk; index++) {
            pre_chunk = jQuery('#' + file.id + "_" + index);
        ...
        var currentProgessBar = jQuery('#' + file.id + "_" + current_uploading_chunk);
...
FileProgress.prototype.setComplete = function(up, info) {
    ...
    var imgWrapper = jQuery('<div class="imgWrapper span5"/>');
    var linkWrapper = jQuery('<a class="linkWrapper" target="_blank"/>');
    var showImg = jQuery('<img src="loading.gif"/>');

    progressNameTd.append(Wrapper);

    if (!isImg) {
        showImg.attr('src', 'components/com_zmaxcdn/images/default.png');
        ...
        jQuery(img).on('load', function() {
            ...
            function initImg(url, key, height) {
                jQuery('#myModal-img').modal();
                var modalBody = jQuery('#myModal-img').find('.modal-body');
                if (height <= 300) {
                    jQuery('#myModal-img').find('.text-warning').show();
                }
            ...
            var infoWrapper = jQuery('<div class="infoWrapper span6"></div>');


            var fopLink = jQuery('<a class="fopLink"/>');
            fopLink.attr('data-key', res.key).text('查看处理效果');
            infoWrapper.append(fopLink);
            fopLink.on('click', function() {
                var key = jQuery(this).data('key');
                var height = parseInt(jQuery(this).parents('.Wrapper').find('.origin-height').text(), 10);
                if (height > jQuery(window).height() - height_space) {
                    height = parseInt(jQuery(window).height() - height_space, 10);
                } else {
                ...
                jQuery('#myModal-img').on('hide.bs.modal', function() {
                    jQuery('#myModal-img').find('.btn-default').removeClass('disabled');
                    jQuery('#myModal-img').find('.text-warning').hide();
                }).on('show.bs.modal', function() {
                    jQuery('#myModal-img').find('.imageView').find('a:eq(0)').addClass('disabled');
                    jQuery('#myModal-img').find('.watermark').find('a:eq(3)').addClass('disabled');
                    jQuery('#myModal-img').find('.text-warning').hide();
                });
